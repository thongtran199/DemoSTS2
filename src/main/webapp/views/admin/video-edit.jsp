					...
					<div class="card-body">
						<div class="form-group">
							<label for="titleInput">Title</label> <input type="text"
								class="form-control" id="titleInput" placeholder="" value="${video != null ? video.title : ''}">
						</div>
						<div class="form-group">
							<label for="hrefInput">Href</label> <input type="text"
								class="form-control" id="hrefInput" placeholder="" value="${video != null ? video.href : ''}">
								<input type="hidden" id="isEdit" value="${isEdit}"/>
								<input type="hidden" id="hrefOrigin" value="${video != null ? video.href : ''}"/>
						</div>
						<div class="row">
							<div class="col-sm-8">
													<div class="form-group">
							<label for="posterHref">Poster</label> <input type="text"
								class="form-control" id="posterInput" placeholder="" value="${video != null ? video.poster : ''}">
						</div>
								<div class="form-group">
									<label>Description</label>
									<textarea id="descInput" class="form-control" rows="3" placeholder="">${video != null ? video.description : ''}</textarea>
								</div>
							</div>
<div class="col-sm-4  d-flex justify-content-end align-items-center">
    <div class="form-group" style="width: 300px; min-height: 175px;overflow:hidden">
        <img
            src="${video != null ? video.poster : '/DemoSTS/templates/admin/img/video-default.png'}"
    ...
    $("#submitBtn").click(function(event) {
        ...
        if(isEdit == "true")
        	...
        	url = "http://localhost:8080/DemoSTS/admin/video?action=edit&href="+$("#hrefOrigin").val();
        	alert(url);
        	}
        else{
        	url="http://localhost:8080/DemoSTS/admin/video?action=add"
        }
        var formData = {
        	 'hrefOrigin': $("#hrefOrigin").val(),
        ...
        $.ajax({
            ...
            window.location.href = "http://localhost:8080/DemoSTS/admin/video?action=view";
        }).fail(function(err) {
            console.log("Oops error: " + err);
        });
    });
    $("#posterInput").change(function() {
        // Update the image source when the poster input changes
        $("#posterImg").attr("src", $("#posterInput").val());
    });
</script>
