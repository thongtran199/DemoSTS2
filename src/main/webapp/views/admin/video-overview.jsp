...
<body class="hold-transition sidebar-mini layout-fixed">
												...
												<table id="example1"
													...
													<thead>
														<tr>
															...
															<th class="sorting" tabindex="0" aria-controls="example1"
																rowspan="1" colspan="1"
																aria-label="Platform(s): activate to sort column ascending">Link</th>
															<th class="sorting" tabindex="0" aria-controls="example1"
																rowspan="1" colspan="1"
													...
													<tbody>
														<c:forEach items="${videos}" var="item" varStatus="loop">
															<c:set var="rowClass"
																value="${loop.index % 2 == 0 ? 'even' : 'odd'}" />
															<tr class="${rowClass}">
																<td class="dtr-control sorting_1" tabindex="0">${item.title}</td>
																<td>${item.description}</td>
																<td><a href="http://localhost:8080/DemoSTS/video?action=watch&id=${item.href}">${item.href}</a></td>
																<td><img src="${item.poster}" style="width:100px; height: 100px;"/></td>
																<td>
																	<div>
																		<button 
																		type="button"
																			class="btn btn-block btn-outline-warning btn-sm"
																			onclick="editBtn('${item.href}')">Sửa</button>
																		<button
																		 type="button"
																			class="btn btn-block btn-outline-danger btn-sm"
																			onclick="deleteBtn('${item.href}')">Xóa</button>
																	</div>
																</td>
															</tr>
														</c:forEach>
  ...
  function deleteBtn(href)
  {
      const deleteURL = "http://localhost:8080/DemoSTS/admin/video?action=delete&href="+href;
  ...
  {
      const editURL = "http://localhost:8080/DemoSTS/admin/video?action=edit&href="+href;
