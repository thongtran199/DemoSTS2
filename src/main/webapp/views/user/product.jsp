		...
		<div class="container">
					...
					<div class="card-body d-flex gap-1 align-items-center">
						...
						<div class="form-group">
							<label for="exampleInputEmail1">Giá bắt đầu</label> <input
								type="number" class="form-control" id="priceBegin"
								placeholder="">
						</div>
						<div class="form-group">
			...
			<div class="row mt-5">
				<div class="col text-center">
					<div class="block-27">
						<ul>
							<c:if test="${currentPage != 1 && maxPage > 1}">
								<li><a
									href="product?action=view&category=${category}&page=${currentPage - 1}">&lt;</a></li>
							</c:if>

							<c:forEach varStatus="i" begin="1" end="${maxPage}">
								<li class="${currentPage == i.index ? 'active' : ''}"><a
									href="product?action=view&category=${category}&page=${i.index}">${i.index}</a></li>
							</c:forEach>
							<c:if test="${currentPage != maxPage && maxPage > 1}">
								<li><a
									href="product?action=view&category=${category}&page=${currentPage + 1}">&gt;
								</a></li>
							</c:if>
						</ul>
					</div>
